...
public class BlockChain {
    ...
    public Map<String, Object> newBlock(long proof, String previous_hash) {
        ...
        block.put("previous_hash", previous_hash != null ? 
        		previous_hash : hash(getChain().get(getChain().size() - 1)));
        // 重置当前的交易信息列表
    ...
    public boolean validChain(List<Map<String, Object>> chain) {
    	Map<String, Object> previousBlock = chain.get(0);
    	int index = 1;
    	while(index < chain.size()) {
    		Map<String, Object> block = chain.get(index);
    		System.out.println(previousBlock.toString());
    		System.out.println(block.toString());
    		System.out.println("\n----------------------------------------------\n");
    		// 检查block的hash是否正确
    		if(!block.get("previous_hash").equals(hash(previousBlock))) {
    			return false;
    		}
    		previousBlock = block;
    		index++;
    ...
    public boolean resolveConflicts() throws IOException {
        ...
        for (String node : neighbours) {
            URL url = new URL("http://" + node + "/myblockchain/chain");
